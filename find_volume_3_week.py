...
def find_abnormal_volume(df, coin):
    ...
    # 分割数据集，最近3周和前10周
    recent_3_weeks = df.tail(3)
    first_10_weeks = df.head(10)
    
   # recent_3_weeks = df.head(3)
   # first_10_weeks = df.tail(9)
    
    # 计算最近3周中最大交易量
    max_volume_recent_3 = recent_3_weeks['volume'].max()
    
    # 找到最大交易量对应的行
    max_volume_row = recent_3_weeks[recent_3_weeks['volume'] == max_volume_recent_3].iloc[0]
    
    # 从该行提取开盘时间
    max_volume_time = max_volume_row['open_time']
    
    # 计算前10周的平均交易量
    avg_volume_first_10 = first_10_weeks['volume'].mean()
    
    # 检查是否存在异常成交量
    if max_volume_recent_3 > 3 * avg_volume_first_10:
        return coin, max_volume_time  # 发现异常成交量，返回币种名称和时间
    else:
        return None, None  # 没有发现异常成交量
...
def check_coins_for_nonnormal_growth(all_coins):
    for coin in all_coins:
        ...
        abnormal_coin, max_volume_time = find_abnormal_volume(daily_data, coin)
        if abnormal_coin:
